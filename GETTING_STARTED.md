...
| `--docker-run` / `--docker-rebuild` | Docker + Docker Compose, **plus**: `example/backend/.env` (from `--bootstrap`) and the mkcert TLS cert above. No GCP service-account key required either — `cli.dart` creates an empty placeholder if you don't have one yet, and reCAPTCHA works the same as native from there (§3 Docker). |
...
`--docker-run`/`--docker-rebuild` refuse to start unless `example/backend/.env` exists (same file `--bootstrap` creates) and the mkcert TLS cert is present — `cli.dart` checks both and hard-errors with the specific one missing. The frontend container serves `https://localhost:8080` and terminates TLS with `example/app/localhost+2.pem`/`-key.pem` (`docker/local/Dockerfile.frontend`) — the SDK's return URI must be HTTPS, same reason `--web` needs it.

The backend container also bind-mounts `example/backend/service-account.json` — Compose has no "skip if missing" option for that, so `cli.dart` auto-creates an empty placeholder if the file doesn't already exist, rather than blocking startup. reCAPTCHA itself is fully `.env`-driven and optional: `RECAPTCHA_SERVICE_ACCOUNT_JSON` accepts inline JSON or a file path (typically `service-account.json`, resolved against the container's mount point — put your real GCP key at `example/backend/service-account.json` on the host for that to work), and leaving all three `RECAPTCHA_*` vars empty disables it. It's never partial though: `config.dart`'s `recaptchaConfigurationErrors`, checked at startup by the same `bin/server.dart` Docker runs, refuses to start if only some of `RECAPTCHA_PROJECT_NUMBER`/`RECAPTCHA_SERVICE_ACCOUNT_JSON`/`RECAPTCHA_SITE_KEY_WEB` are set — that state would otherwise render the client widget while silently never enforcing it server-side.
...
| `--release-dart-patch` / `--release-dart-minor` / `--release-dart-major` | Bumps `zenpay_dart`'s version, re-syncs its example, and commits the bump (scoped to exactly those files, never `git add -A`). Never tags or publishes — `dart pub publish --dry-run`, then `dart pub publish`, are still manual. | Preparing a `zenpay_dart` release. |
