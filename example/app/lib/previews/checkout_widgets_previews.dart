...
@Preview(name: 'AUD, 4 presets', group: 'ZenPayAmountField', size: Size(360, 260))
...
Widget zenpaySelectableCardSelectedPreview() {
  return ZenPaySelectableCard(icon: Icons.credit_card, label: 'Make Payment', subtitle: 'Standard one-off transaction', selected: true, onTap: () {});
}

/// Preview for unselected [ZenPaySelectableCard].
@Preview(name: 'Unselected', group: 'ZenPaySelectableCard', size: Size(200, 120))
Widget zenpaySelectableCardUnselectedPreview() {
  return ZenPaySelectableCard(icon: Icons.lock_outline, label: 'Tokenise', subtitle: 'Save card details for later', selected: false, onTap: () {});
}
...
Widget zenpayLabeledFieldPreview() {
  return ZenPayLabeledField(controller: TextEditingController(), label: 'Customer name', hintText: 'Ada Lovelace');
}
...
@Preview(name: 'Sandbox hosts', group: 'ZenPayEnvironmentBanner', size: Size(390, 60))
Widget zenpayEnvironmentBannerPreview() {
  return const ZenPayEnvironmentBanner(allowedCheckoutHosts: <String>{'pay.sandbox.travelpay.com.au'});
}
