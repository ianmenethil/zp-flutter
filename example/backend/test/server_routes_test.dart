...
AppConfig _config({int checkoutRateLimitPerMinute = 1000, String recaptchaProjectNumber = '', String recaptchaServiceAccountJson = ''}) => AppConfig(
  port: 0,
  ...
  zenPay: ZenPayConfig(
    ...
    credentials: const ZenPayCredentials(merchantCode: 'merchant-code', apiKey: _apiKey, username: _username, password: _password),
  ),
);
...
String _sign(int mode, String amountField, String mupid, String reference) =>
    createSha3_512([_apiKey, _username, _password, mode.toString(), amountField, mupid, reference].join('|'));
...
String _token({required String merchantUniquePaymentId, int mode = 0, num amount = 49.90}) => createZpCallbackUrlToken(
...
Map<String, Object?> _prepareBody({int mode = 0, num? paymentAmount = 10, String customerEmail = 'jane@example.com'}) => {
...
void main() {
  ...
  Future<void> startServer(AppConfig testConfig, {RecaptchaVerifier? recaptchaVerifier}) async {
    config = testConfig;
    store = AttemptStore();
    server = await shelf_io.serve(buildHandler(config, store, recaptchaVerifier: recaptchaVerifier), InternetAddress.loopbackIPv4, 0);
  ...
  Future<http.Response> call(String path, {String method = 'GET', Map<String, String>? headers, Object? body}) async {
  ...
  Future<http.Response> prepare({Map<String, Object?>? body, String idempotencyKey = 'idempotency-key-prepare-0001', String client = 'web'}) => call(
    '/api/v1/checkout/token',
    method: 'POST',
    headers: {'content-type': 'application/json', 'idempotency-key': idempotencyKey, 'x-client': client},
    body: body ?? _prepareBody(),
  );

  Future<http.Response> exchange(String checkoutToken) =>
      call('/api/v1/checkout/exchange', method: 'POST', headers: {'authorization': 'Bearer $checkoutToken'});
  ...
  Future<({String checkoutToken, String merchantUniquePaymentId, Map<String, Object?> exchanged})> freshCheckout({
    Map<String, Object?>? body,
    String idempotencyKey = 'idempotency-key-prepare-0001',
  }) async {
    final prepareResponse = await prepare(body: body, idempotencyKey: idempotencyKey);
  ...
  group('POST /api/v1/checkout/token', () {
    ...
    test('rejects a missing or unknown X-Client header', () async {
      final missing = await call(
        '/api/v1/checkout/token',
        method: 'POST',
        headers: {'content-type': 'application/json', 'idempotency-key': 'idempotency-key-no-client-header'},
        body: _prepareBody(),
      );
    ...
    test('mode 1 (Tokenise) does not require a paymentAmount', () async {
      final response = await prepare(body: _prepareBody(mode: 1, paymentAmount: null));
      expect(response.statusCode, 201);
    });

    test('mode 1 without a paymentAmount omits it from the launch URL', () async {
      final result = await freshCheckout(body: _prepareBody(mode: 1, paymentAmount: null), idempotencyKey: 'idempotency-key-tokenise-unpriced');
    ...
    test('mode 1 with a paymentAmount passes it through for fee display', () async {
      final result = await freshCheckout(body: _prepareBody(mode: 1, paymentAmount: 999), idempotencyKey: 'idempotency-key-tokenise-priced');
      final checkoutUrl = Uri.parse(result.exchanged['checkoutUrl']! as String);
      expect(checkoutUrl.queryParameters['paymentAmount'], '999');
    });

    test('mode 2 (Custom Payment) requires a positive paymentAmount', () async {
      final missing = await prepare(body: _prepareBody(mode: 2, paymentAmount: null));
      expect(missing.statusCode, 400);
      expect(jsonDecode(missing.body), {'error': 'INVALID_CHECKOUT_AMOUNT'});

      final ok = await prepare(body: _prepareBody(mode: 2, paymentAmount: 15.5));
      expect(ok.statusCode, 201);
    });

    test('mode 3 (Pre-Auth) requires a paymentAmount, same as mode 0', () async {
      final response = await prepare(body: _prepareBody(mode: 3, paymentAmount: null));
      expect(response.statusCode, 400);
      expect(jsonDecode(response.body), {'error': 'INVALID_CHECKOUT_AMOUNT'});
    });

    test('a repeated Idempotency-Key re-mints a token for the same attempt', () async {
      final first = await freshCheckout(idempotencyKey: 'idempotency-key-dup');
      final secondPrepare = await prepare(idempotencyKey: 'idempotency-key-dup');
      ...
      expect(secondPayload.merchantUniquePaymentId, first.merchantUniquePaymentId);
    });
    ...
    test('rejects a checkout-creation burst once the per-IP limit trips', () async {
      await server.close(force: true);
  ...
  group('POST /api/v1/checkout/exchange', () {
    ...
    test('replaying the same checkout token resolves to the same attempt, not a new one', () async {
      final first = await freshCheckout(idempotencyKey: 'idempotency-key-replay');
  ...
  group('GET /api/v1/sessions', () {
    test('returns the stored attempt state for a valid token, with no identifiers', () async {
  ...
  group('POST /api/v1/callbacks', () {
    test('accepts a correctly signed callback and updates status', () async {
      store.create(_attempt(merchantUniquePaymentId: 'att-cb-1', amount: 25.50));
      ...
      final response = await call(
        ...
        body: {
          'response': {'merchantUniquePaymentId': 'att-cb-1', 'paymentReference': 'PAY-1', 'paymentStatus': 3},
    ...
    test('rejects a tampered ValidationCode', () async {
      store.create(_attempt(merchantUniquePaymentId: 'att-cb-2', amount: 25.50));
      ...
      final response = await call(
        ...
        headers: {'content-type': 'application/json'},
        body: {
          'response': {'merchantUniquePaymentId': 'att-cb-2', 'paymentReference': 'PAY-2', 'paymentStatus': 3},
          'validationCode': wrongDigest,
        },
      );
      expect(response.statusCode, 401);
      expect(jsonDecode(response.body), {'error': 'CALLBACK_VALIDATION_FAILED'});
    ...
    test('404s for an unknown attempt', () async {
      ...
      expect(response.statusCode, 404);
      expect(jsonDecode(response.body), {'error': 'CALLBACK_ATTEMPT_NOT_FOUND'});
    });
  });

  group('GET /return', () {
    test('redirects a mobile attempt to the app-return App Link', () async {
      store.create(_attempt(merchantUniquePaymentId: 'att-return-mobile'));
    ...
    test('redirects a web attempt to the configured web return origin', () async {
      store.create(_attempt(merchantUniquePaymentId: 'att-return-web', client: CheckoutClient.web));
      final token = _token(merchantUniquePaymentId: 'att-return-web');
      final request = http.Request('GET', Uri.parse('$base/return?t=$token'))..followRedirects = false;
      final response = await http.Response.fromStream(await request.send());
      expect(response.statusCode, 303);
      expect(response.headers['location'], startsWith('https://localhost:3000/'));
    });
    ...
    test('400s for a token naming an unknown attempt', () async {
      ...
      expect(response.statusCode, 400);
      expect(jsonDecode(response.body), {'error': 'RETURN_TOKEN_UNKNOWN_ATTEMPT'});
    });
  });

  group('reCAPTCHA enforcement', () {
    test('rejects POST /api/v1/checkout/token with 401 when header is missing', () async {
      await server.close(force: true);
      final verifier = _FakeRecaptchaVerifier();
      await startServer(_config(recaptchaProjectNumber: '123456789'), recaptchaVerifier: verifier);

      final response = await prepare(idempotencyKey: 'idempotency-key-appcheck-missing');

      expect(response.statusCode, 401);
      expect(jsonDecode(response.body), {'error': 'RECAPTCHA_TOKEN_MISSING'});
    });

    test('rejects POST /api/v1/checkout/token with 401 when token is invalid', () async {
      await server.close(force: true);
    ...
    test('accepts POST /api/v1/checkout/token with 201 when token is valid', () async {
      await server.close(force: true);
      final verifier = _FakeRecaptchaVerifier();
      await startServer(_config(recaptchaProjectNumber: '123456789'), recaptchaVerifier: verifier);
    ...
    test('skips reCAPTCHA entirely for x-client: mobile, even without a token', () async {
      await server.close(force: true);
      final verifier = _FakeRecaptchaVerifier(shouldPass: false);
      await startServer(_config(recaptchaProjectNumber: '123456789'), recaptchaVerifier: verifier);

      final response = await prepare(idempotencyKey: 'idempotency-key-mobile-no-recaptcha', client: 'mobile');

      expect(response.statusCode, 201);
    });

    test('skips reCAPTCHA verification when recaptchaProjectNumber is unconfigured', () async {
      await server.close(force: true);
      final verifier = _FakeRecaptchaVerifier(shouldPass: false);
      await startServer(_config(), recaptchaVerifier: verifier);

      final response = await prepare(idempotencyKey: 'idempotency-key-appcheck-bypassed');
  ...
  group('http_trace header redaction', () {
    test('masks only authorization and x-recaptcha-token in logged headers', () async {
      ...
      final response = await call(
        ...
        method: 'POST',
        headers: {'authorization': 'Bearer super-secret-checkout-token', 'x-recaptcha-token': 'app-check-jwt-secret-value', 'x-request-id': 'trace-me-123'},
      );
      expect(response.statusCode, 401);

      await subscription.cancel();
      final traces = records.map((r) => r.message).where((m) => m.contains('http_trace')).toList();
      final exchangeTrace = traces.singleWhere((m) => (jsonDecode(m) as Map<String, Object?>)['path'] == '/api/v1/checkout/exchange');
    ...
    test('masks checkoutToken in logged response bodies', () async {
      ...
      final response = await prepare(idempotencyKey: 'idempotency-key-redaction-body');
      expect(response.statusCode, 201);
      final token = (jsonDecode(response.body) as Map<String, Object?>)['checkoutToken']! as String;

      await subscription.cancel();
      final traces = records.map((r) => r.message).where((m) => m.contains('http_trace')).toList();
      final tokenTrace = traces.singleWhere((m) => (jsonDecode(m) as Map<String, Object?>)['path'] == '/api/v1/checkout/token');
      final body = (jsonDecode(tokenTrace) as Map<String, Object?>)['responseBody']! as Map<String, Object?>;

      expect(body['checkoutToken'], '${token.substring(0, 3)}...${token.substring(token.length - 3)}');
    ...
    test('masks token and zp_session cookies, leaves other cookies as-is', () async {
      ...
      final response = await call(
        '/api/v1/checkout/exchange',
        method: 'POST',
        headers: {'cookie': 'zp.last_used_login_method=email; zp_session=super-secret-session-value; token=super-secret-token-value'},
      );
      expect(response.statusCode, 401);
      ...
      expect(headers['cookie'], 'zp.last_used_login_method=email; zp_session=sup...lue; token=sup...lue');
    });
  });
}
