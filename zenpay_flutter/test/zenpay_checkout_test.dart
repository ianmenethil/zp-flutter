...
final class _FakeCheckoutPresenter extends CheckoutPresenter {
  ...
  Future<PresentationLaunchResult> openCheckout(Uri url, {required bool showTitle, required bool allowExternalBrowserFallback}) async =>
      PresentationLaunchResult(launched: launched, usedExternalBrowserFallback: false);
...
final class _HangingCheckoutPresenter extends CheckoutPresenter {
  ...
  @override
  Future<PresentationLaunchResult> openCheckout(Uri url, {required bool showTitle, required bool allowExternalBrowserFallback}) => openCompleter.future;
...
void main() {
  group('mapLaunchFailureCode', () {
    test('MissingPluginException maps to platformUnavailable', () {
      expect(mapLaunchFailureCode(MissingPluginException()), ZpLaunchFailureCode.platformUnavailable);
    });

    test('any other error maps to platformError', () {
      expect(mapLaunchFailureCode(Exception('boom')), ZpLaunchFailureCode.platformError);
    });
  });
  ...
  group('ZpCheckout', () {
    ...
    setUp(() {
      ...
      checkout = ZpCheckout(configuration: configuration, returnUriSource: returnUriSource, presenter: presenter);
    });
    ...
    test('a valid return URI completes with ZpReturnReceived', () async {
      final outcome = checkout.open(checkoutUrl: Uri.parse('https://checkout.example.com/pay?secureToken=abc'));

      returnUriSource.emit(Uri.parse('https://app.example.com/return?merchantUniquePaymentId=order-1'));
    ...
    test('a presenter dismissal event completes with ZpPresentationDismissed', () async {
      final outcome = checkout.open(checkoutUrl: Uri.parse('https://checkout.example.com/pay?secureToken=abc'));
    ...
    test('a disallowed checkout host throws ZpInvalidLaunchException', () {
      expect(() => checkout.open(checkoutUrl: Uri.parse('https://not-allowed.example.com/pay')), throwsA(isA<ZpInvalidLaunchException>()));
    });
    ...
    test('a stale initial return is not replayed into a second, unrelated open() call', () async {
      final sourceWithInitial = FakeReturnUriSource(initialUri: Uri.parse('https://app.example.com/return?merchantUniquePaymentId=order-A'));
      final localCheckout = ZpCheckout(configuration: configuration, returnUriSource: sourceWithInitial, presenter: presenter);

      final firstOutcome = await localCheckout.open(checkoutUrl: Uri.parse('https://checkout.example.com/pay?secureToken=a'));
      expect(firstOutcome, isA<ZpReturnReceived>());
      expect((firstOutcome as ZpReturnReceived).returnUri.queryParameters['merchantUniquePaymentId'], 'order-A');

      final secondOutcomeFuture = localCheckout.open(checkoutUrl: Uri.parse('https://checkout.example.com/pay?secureToken=b'));
      await pumpEventQueue();

      // Only payment B's own return should ever resolve this attempt.
      sourceWithInitial.emit(Uri.parse('https://app.example.com/return?merchantUniquePaymentId=order-B'));
      final secondOutcome = await secondOutcomeFuture;

      expect(secondOutcome, isA<ZpReturnReceived>());
      expect((secondOutcome as ZpReturnReceived).returnUri.queryParameters['merchantUniquePaymentId'], 'order-B');
    ...
    test('dispose() while openCheckout() is still pending does not drop a return that arrives before openCheckout() resolves', () async {
      final hangingPresenter = _HangingCheckoutPresenter();
      final source = FakeReturnUriSource();
      final localCheckout = ZpCheckout(configuration: configuration, returnUriSource: source, presenter: hangingPresenter);

      final outcomeFuture = localCheckout.open(checkoutUrl: Uri.parse('https://checkout.example.com/pay?secureToken=abc'));
      ...
      source.emit(Uri.parse('https://app.example.com/return?merchantUniquePaymentId=order-1'));
      await pumpEventQueue();
      expect(hangingPresenter.dismissCallCount, 1);

      hangingPresenter.openCompleter.complete(const PresentationLaunchResult(launched: true, usedExternalBrowserFallback: false));
