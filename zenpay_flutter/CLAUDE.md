...
- **[lib/src/return_handling/](lib/src/return_handling/)**: Deep-link ingestion and return sanitization — really only 2 problems split across platform files, not 9 independent things. (1) Mobile and Web need different mechanisms to detect a return happened: a shared `ZpReturnUriSource` contract + factory, a mobile implementation (`app_links`), a web implementation (popup listener), and a shared validator. (2) Web-only: checkout opens in a *second browser tab*, so that tab has to relay the return back to the tab that opened it — that's the `web_popup_*` + `web_return_protocol.dart` files below.
  ...
  - **[web/web_return_protocol.dart](lib/src/return_handling/web/web_return_protocol.dart)**: Pure-Dart `postMessage` handoff protocol — encoding/decoding return payloads plus origin/address validation for incoming web messages. Kept `dart:js_interop`-free so this security-relevant logic stays unit-testable on the VM.
