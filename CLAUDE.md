...
- **`_bootstrap(...)`**: First-run setup on a fresh clone, leaving nothing to hand-edit — resolves the pub workspace (`dart pub get`), creates `example/backend/.env` and `example/app/.env` from their `.example` templates, prompts for each value the backend refuses to start without (`ZENPAY_MERCHANT_CODE`, `ZENPAY_API_KEY`, `ZENPAY_USERNAME`, `ZENPAY_PASSWORD`, `TOKEN_SECRET` — Enter keeps what's already set, and a blank `TOKEN_SECRET` is auto-generated), generates the local mkcert TLS cert `example/app` needs for the web return flow (`--skip-certs` to skip), and writes `example/backend/well_known/assetlinks.json` for this machine. Deliberately does **not** prompt for `PUBLIC_BASE_URL` — `_server` owns that, and it changes every time a tunnel restarts.
